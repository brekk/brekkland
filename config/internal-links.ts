...
import type { WikilinkMatch, Project, LinkMatch } from "@/types"
import type {
  Predicate,
  StringPred,
  NullString,
...
// globalPostsCache
export function closured(_cache: any[] = []) {
  let cache = _cache
  const set = (x: any[]) => {
    cache = x
  }
...
export function decodeAnchor(y: NullString): NullString {
  return pipe(defaultTo(""), (x: string) =>
    tryCatch(decodeURIComponent, K(x))(x),
  )(y)
}

const trace = curryN(2, (msg: string, x: any) => {
  console.log(msg, x)
  return x
})

export const parseLinkWithAnchor = (link: string): AnchoredLink =>
  pipe(
    indexOf("#"),
    ifElse(
      equals(-1),
      K({ link, anchor: null }),
      pipe<number[], [string, NullString], AnchoredLink>(
        bisect($, link) as any,
        ([link, b]) => ({ link, anchor: b ? decodeAnchor(b) : null }),
      ),
    ),
  )(link)

export const isCode = (v: string) => v === "inlineCode" || v === "code"

export const isInCodeBlock = (parent: any): boolean => {
  if (!parent) {
    return false
  } else {
    const { type } = parent
...
export const searchWithinByIndex = curryN(
  3,
  (content: string, index: number, regex: RegExp) => {
    let $match
...
export const isWikilinkInCode = (
  content: string,
  wikilinkIndex: number,
): boolean => {
  const check = searchWithinByIndex(content, wikilinkIndex)
  return check(REGEX.CODE) || check(REGEX.BACKTICK)
}

export const pathStartsWith = curryN(
  2,
...
export const notARealLink = (x: string): boolean =>
  nonePass(
    [
      startsWith("//"),
      startsWith("http:"),
...
export const dropEntityMd = curryN(2, (v: string, x: string): string =>
  pipe(collapse(v), collapse(/\.md$/))(x),
)
...
export const isIndexPath = both(
  endsWith("/index"),
  pipe(split("/"), length, equals(2)),
)

export const indexHandle = (x: string): string =>
  when<string, string>(isIndexPath, collapse("/index"))(x)

export const processLink = (x: string): string =>
  cond([
    [
...
export const extractLinkTextFromAnchor = (url: string): AnchoredLink =>
...
// ============================================================================
//
const createSlugFromTitle = slug

const createAnchorSlug = slug

const mapRelativeUrlToSiteUrl = pipe(
  // relativeUrlMapper,
  makeRelativeUrl,
  (x) => import.meta.env.BASE_URL + x,
)

// Helper function to extract link text and anchor from URL for internal links
function extractLinkTextFromUrlWithAnchor(url: string): {
        ...
        if (linkText) {
          ...
          // Convert .md file references to proper URLs based on collection
            ...
            if (linkText === "homepage") {
              ...
              strippedUrl.startsWith("projects/") ||
              strippedUrl.startsWith("/projects/")
            ) {
              // Projects: /projects/slug/
              let projectPath = strippedUrl.replace(/^\/?projects\//, "")
              if (
                projectPath.endsWith("/index") &&
              ...
              strippedUrl.startsWith("docs/") ||
              strippedUrl.startsWith("/docs/")
            ) {
              // Docs: /docs/slug/
              let docPath = strippedUrl.replace(/^\/?docs\//, "")
              if (
                docPath.endsWith("/index") &&
                docPath.split("/").length === 2
              ) {
              ...
              if (file && (file as any).path) {
                const normalizedPath = String((file as any).path).replace(
                  /\\/g,
                  "/",
                )
